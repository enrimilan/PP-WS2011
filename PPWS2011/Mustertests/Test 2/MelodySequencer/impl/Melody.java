...
public class Melody {
	...
	public Melody copy(int beginIndex, int endIndex) {
		...
		Melody melody1 = new Melody(bpm);
		for(int i = beginIndex;i<endIndex;i++){
		melody1.addNote(new Note(melody.get(i)));
		}
		return melody1;
	...
	public Melody copy(int beginIndex) {
		return copy(beginIndex, melody.size());
	...
	public String toString() {
		...
		int dauer = 0;
		for(int i = 0 ; i<melody.size();i++){
		out=out+melody.get(i)+" ";
		dauer=dauer+melody.get(i).getBeats();
		}
		return out+"\n"+((60.0/(double) bpm)*(double)dauer)+" seconds";
